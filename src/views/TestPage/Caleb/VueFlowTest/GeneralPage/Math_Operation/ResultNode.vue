...
import { Position, useNodeConnections, useNodesData } from '@/lib/lib_vueFlow'
...
const operatorData = useNodesData(() =>
  sourceConnections.value.map((connection: any) => connection.source)
)

const valueData = useNodesData(() =>
  operatorSourceConnections.value.map((connection: any) => connection.source)
)
