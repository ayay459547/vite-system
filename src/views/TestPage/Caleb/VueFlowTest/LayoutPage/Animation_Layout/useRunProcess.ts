...
export function useRunProcess({ graph: dagreGraph, cancelOnError = true }) {
  ...
  async function runNode(nodeId, isStart = false) {
    ...
    const incomers = getConnectedEdges(nodeId).filter(
      (connection: any) => connection.target === nodeId
    )

    // wait for edge animations to finish before starting the process
    await Promise.all(
      incomers.map((incomer: any) => until(() => !incomer.data?.isAnimating))
    )
      ...
      const timeout = setTimeout(
        async () => {
          ...
          if (children.length > 0) {
            ...
            await Promise.all(children.map((child: any) => runNode(child)))
          }
  ...
  async function run(nodes) {
    ...
    const startingNodes = nodes.filter(
      (node: any) => graph.value.predecessors(node.id)?.length === 0
    )

    // run the process on all starting nodes in parallel
    await Promise.all(startingNodes.map((node: any) => runNode(node.id, true)))
