...
export const formatParams = (params: any): Params => {
  ...
  let timeLineRangeFilter = {}
  const [_startDate, _endDate] = Array.isArray(timeLineRange) ? timeLineRange : []
  if (!isEmpty(_startDate) && !isEmpty(_endDate)) {
    timeLineRangeFilter = {
      isStartDate_predefinedstartdatetime: _startDate,
      // isStartDate_predefinedStartDate: _startDate,
      isEndDate_predefinedCompleteDate: _endDate
    }
  }
