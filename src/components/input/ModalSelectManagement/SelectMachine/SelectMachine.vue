...
import { useMachineInfo } from '@/declare/declare_machine/useMachineInfo'
import { columnSetting, linkSetting } from '@/declare/declare_machine/columns'
...
<template>
  <div class="i-page">
    <WebViewTable
      ...
      :fake-data="[]"
      fake-data-path="/declare/machine_fakeData.json"
    ...
    >
      ...
      <template #column-hasWorkingTime="{ data } ">
        <div class="flex-row-center i-ga-md">
          <CustomIcon
            :name="data === 1 ? 'check' : 'xmark'"
            :class="data === 1 ? 'text-success' : 'text-info'"
          />
        </div>
      </template>
    </WebViewTable>
