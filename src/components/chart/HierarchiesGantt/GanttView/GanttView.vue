...
import { SimpleFilter, CustomInput, CustomButton, CustomTabs, CustomDivider } from '@/components'
...
const resetCanvas = async (type) => {
  await nextTick()
  ganttObject?.value.reset(type)
}

const initOrders = async () => {
  await nextTick()
  ganttObject?.value.reset('yItems')
}
...
const setLevel = async (type, data?) => {
  await nextTick()
  ganttObject?.value.setLevelInfo(type, data)
  init()
}
const setXScroll = (value, type) => {
  ganttObject?.value.setXScroll(value, type)
  resetCanvas('xScale')
  // init()
}
const setYScroll = (value, type) => {
  ganttObject?.value.setYScroll(value, type)
  // init()
}
const setScrollByOverView = (newTime, newYRate) => {
  ganttObject?.value.setXScroll(newTime, 'timeValue')
  ganttObject?.value.setYScroll(newYRate, 'rate')
}
const setAdjustTime = () => {
  init()
  ganttObject?.value.setAdjustTime()
  // init()
}

const getXAxisGrids = () => {
  return ganttObject?.value.getXAxisGrids()
}
const getFocusX = time => {
  return ganttObject?.value.getFocusX(time)
}
const getFocusTime = type => {
  return ganttObject?.value.getFocusTime(type)
}
...
//   if(isEmpty(ganttObject.value)) return
//   // ganttObject.value.init()
//   ganttObject.value.reset('canvasSize')
//   ganttObject.value.drawFrame()
//   // init()
// }
...
const ganttClick = (event) => {
  ganttObject.value.checkEvent(event)
...
onMounted(() => {
 ...
 // 設定甘特圖
  ganttObject.value = createGanttView(
    ganttCanvas?.value,
    props.ganttData,
    props.dragItem,
