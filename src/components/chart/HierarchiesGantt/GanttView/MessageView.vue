...
// import { CustomPopover, SimpleFilter, CustomInput, CustomButton, CustomTabs, CustomDivider } from '@/components' // 系統組件
// import { isEmpty, message } from '@/lib/lib_utils' // 工具
          ...
          <template v-for="(item, index) in props.message.data" :key="index">
            <template v-if="index === 0">
              <div class="popover-data orders">
                <span> {{ `${i18nTranslate('manufacturing-order-no')} :` /*工單編號*/ }} </span>
                <span> {{ `${item.orderIds}` }} </span>
              </div>
              <div class="popover-data orders">
                  <span> {{ `${i18nTranslate('routing-orderIndex')} :` /*途程站點順序*/ }} </span>
                  <span> {{ `${item.moiIndex}` }} </span>
                </div>
              <div class="popover-data orders">
                <span> {{ `${i18nTranslate('manufacturing-order-status')} :` /*工單狀態*/ }} </span>
        ...
        <div div class="popover-container orders">
          <template v-for="(item, index) in props.message.data" :key="index">
            <template v-if="index === 0">
              ...
              <div class="popover-data orders">
                <span> {{ `${i18nTranslate('manufacturing-order-status')} :` /*工單狀態*/ }} </span>
                <span> {{ i18nTranslate('rushOrder') /*'插單'*/ }} </span>
