...
import { isEmpty,deepClone, message } from '@/lib/lib_utils' // 工具
...
export const createGanttView = (canvasRef, ganttData, dragItem, rushManagement, sideView, objectController, canvasMessage, cursorType, useFunction) => {
  ...
  const createYAxis = () => {
    ...
    const yAxisHeader = {
      ...
      draw () {
        ...
        const drawColumns = () => {
          ...
          const columnscount = flatYItems.length
          const ordesCount = flatYItemsOrders.reduce((count, orders) => {
          ...
          switch (ganttData.mode) {
            case 'edit': {
              ...
              const label = objectController.i18n.getTranslate('gantt-machine-rushable') //'可分配機台編號'
              const text = `${label} (${columnscount},${ordesCount})`
    ...
    const yAxisScrollArea = {
      ...
      draw () {
        ...
        for(let i = startIndex; i <= endIndex ; i++) {
          ...
          const textY = itemY + effectColumnHeight * 0.5 + 2
          ...
          const drawItem = {
            ...
            yAxisText: () => {
              const ordersCount = (() => {
                switch(ganttData.mode) {
                  case 'view':
                  case 'edit': return flatYItemsOrders[i].length
                  case 'result': return flatYItemsOrders[i].filter(order => {
                    if(order.isNew) return true
                    return !isEditingOrder(order)
                    // if(ganttData.ed)
                    // console.log(order)
                  }).length
                }
              ...
              const text = `${item.name} (${ordersCount}) `
              if(item.type === 'group') {
                ctx.save()
          ...
          switch (type) {
            ...
            case 'scroll': {
              ...
              const getOrders = () => {
                ...
                const orders = []
                for (let i = startIndex; i <= endIndex ; i ++ ) {
                  if(flatYItemsOrders[i]) orders.push(flatYItemsOrders[i])
                }
                return orders.flat()
              }
              const timeRange = getTimeRange()
              ...
              const yItemOrders = getOrders() // [ ... new Set(yItemList.map(yItem => getItemOrders(yItem)).flat())]
              // console.log(yItemOrders)
    ...
    const setOrderLink = () => {
      ...
      switch(ganttData.mode) {
        ...
        case 'result':{
          const fromEditItems = flatYItems.map((item, index) => {
            const orders = flatYItemsOrders[index]
    ...
    const setFlatYItems = () => {
      ...
      const flatYItemArray = Array.from(yItems)
      // console.time('APPLYFilter')

      // console.log('SETYITEM', yItems, flatYItemArray)
      ...
      const sortArray  = unsortArray.sort((a, b) => {
        const timeA = isEmpty(a.orders) ? -1 : a.orders[0].startTimeValue
        const timeB = isEmpty(b.orders) ? -1 : b.orders[0].startTimeValue
        return timeA - timeB
      })
  ...
  const createXAxis = () => {
    ...
    //     const startY = canvasY + areaHeight - levelHeight * (index + 2)
    ...
    const drawArea = () => {
        ...
        // upperLevels.splice(0)
        // upperLevelIndexs.value.forEach(index => upperLevels.push(timeGroups[index]))
        ...
        // upperLevelsItems.splice(0)
        // upperLevels.forEach(upperLevel => upperLevelsItems.push(setUpperItems(upperLevel)))
        const upperLevelsItems = upperLevels.map(upperLevel => setUpperItems(upperLevel))
        const baseItems = setBaseItems(baseLevel)

        const drawUpperItems = (upperLevelsItems) => {
          ctx.save()
          setStyle('xAxisUpperText')
          setStyle('standardLine')
        ...
        //     // const lineHeight = levelHeight
        //      // text
        //     // const text = baseLevel.baseText(item.startTime)
        //     // const textX = item.startX + item.width / 2
        //     // const textY = startY + levelHeight / 2 + 1

        //     if(isHoverTimeRange(item, baseLevelIndex.value)) {
        //       ctx.save()
        //       clipRect(item.startX, startY, item.width, levelHeight)
        //       setStyle('hoverBaseTimeBack')
        //       drawRect(item.startX, startY, item.width, levelHeight, 'fill')
        //       setStyle('hoverTimeEffect')
        //       drawRect(item.startX, startY, item.width, levelHeight, 'stroke')
        //       ctx.restore()
        //     }
        //     setStyle('standardLine')
        //     drawLine(item.startX, startY, 0, levelHeight)
        //     // drawText(text, textX, textY)
        ...
        drawUpperItems(upperLevelsItems)
        drawBaseItems(baseItems)
        ...
        const drawTimeMarks = () => {
          ganttData.timeMarks.forEach(timeMark => {
            ...
            const markX = computeXAxisCoordinate(timeMark.timeString, 'string')
            ctx.strokeStyle = timeMark.color
            drawLine(markX, borderY, 0, areaHeight + gridHeight)
            if (timeMark.displayHeader) {
              const textWidth = ctx.measureText(timeMark.label).width
              ctx.fillStyle = timeMark.color
              ...
              ctx.fillStyle = 'white'
              drawText(timeMark.label, markX + 5, borderY + levelHeight * 0.5)
            }
          })
        }
    ...
    const setAdjustTime = timeRange => {
      ...
      // console.log(ganttGrid.getSize().width)
  ...
  let requstId
  const drawCanvas = () => {
    if(!objectController.checkEnd()) {
      objectController.canvasResize()
      drawFrame()
      requstId = requestAnimationFrame(drawCanvas)
    }
    else {
      // clearTimeout(requstId)
  ...
  // const drawCanvas2 = () => {
  //   // 同一個Function不斷requestFrame時會有記憶體堆疊的現象
  //   // Function一段時間內沒被呼叫的話這些記憶體會被釋放
  //   // 故用drawCanvas,drawCanvas2輪流呼叫的方式清除記憶體
  //   if(!objectController.checkEnd()) {
  //     objectController.canvasResize()
  //     drawFrame()
  //     const cur = Date.now()
  //     if((cur % 90000) > 45000) {
  //       requstId = requestAnimationFrame(drawCanvas2)
  //     }
  //     else {
  //       cancelAnimationFrame(requstId)
  //       requstId = requestAnimationFrame(drawCanvas)
  //     }
  //     // console.log('Draw')
  //     // requstId = requestAnimationFrame(drawFrame)
  //   }
  //   else {
  //     // clearTimeout(requstId)
  //     cancelAnimationFrame(requstId)
  ...
  const drawFrame = () => {
    ...
    drawRect(0, 0, width, height, 'fill')
    // ctx.clearRect(0, 0, width, height)
    ganttGrid.drawArea() // V2
    yAxis.drawArea() // X
    xAxis.drawArea() // X
