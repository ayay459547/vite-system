...
import { ref, inject, nextTick, reactive, computed, onMounted, onUnmounted, onBeforeMount } from 'vue'
...
import { deepClone, isEmpty, getUuid, hasOwnProperty } from '@/lib/lib_utils' // 工具
...
const i18nBusListener = (event: string, value) => {
  switch (event) {
    case 'langChange':
...
const i18nKeyMap: Map<string, string> = new Map()
const getTranslate = (key: string) => {
  if(i18nKeyMap.has(key)) return i18nKeyMap.get(key)
  else return setTranslate(key)
}
const setTranslate = (key: string) => {
  const translate = i18nTest(key) ? i18nTranslate(key) : key
  i18nKeyMap.set(key, translate)
  return translate
}
const resetTranslateMap = () => {
  const i18nKeys = [...i18nKeyMap.keys()]
  i18nKeys.forEach(key => setTranslate(key))
}
...
const inSideController: any = {
  ganttData: {
    ...
    addTimeMark: data => {
      ...
      if(timeMark) {
        const timeString = timeMark.timeString
        setXScroll(timeString, 'timeString')
      }
      else {
        ganttData.value.timeMarks.push(data)
        setXScroll(data.timeString, 'timeString')
      }
      //sort
    },
    findTimeMark: data => {
    ...
    resetTempRootYItem: async yItem => {
    ...
    applySearch(groupItem, levelIndex, levelFilter) {
      ...
      const setItem = item => {
        if(item.i18nLabel) {
          ...
          item.name = getTranslate(item.i18nLabel)
        }
        return item
      }
...
<template>
  <template v-if="useFunction.OverView">
      <!-- <CustomModal
