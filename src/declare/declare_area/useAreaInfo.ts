...
import type { Params, ResponseData, ExcelData, TableData } from '@/types/types_area'

export const useAreaInfo = (options: {
  i18nModule?: ScopeKey
}) => {
  ...
  const formatParams = (params: any): Params => {
    const { areaId, areaName, areaCategory, parentNo, lastUpdateDate } = params
  ...
  const formatData = (row: ResponseData): ExcelData | TableData => {
    const { areaId, lastUpdateDate } = row
